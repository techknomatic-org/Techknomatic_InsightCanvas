        ...
        <ClickAwayListener onClickAway={handleClickAway} mouseEvent="onMouseDown">
            ...
            >
                ...
                <Box sx={{
                    ...
                    {/* Primary action — Circular blue + button matching reference design */}
                        ...
                        <IconButton
                            ...
                            sx={{
                                ...
                                '&:hover': {
                                    ...
                                    boxShadow: '0 4px 12px rgba(27, 117, 187, 0.5)',
                                },
                            }}
                        >
                ...
                {isOpen && (
                    <Box sx={{
                        ...
                        overflow: 'hidden',
                        borderRight: '1px solid rgba(27, 117, 187, 0.12)',
                        boxShadow: isPinned
                            ? '6px 0 24px -2px rgba(27, 117, 187, 0.14), 2px 0 8px rgba(15, 23, 42, 0.05)'
                            : '12px 0 36px -4px rgba(27, 117, 187, 0.22), 4px 0 14px rgba(15, 23, 42, 0.08)',
                        zIndex: 10,
                    }}>
...
const DataSourceSidebarPanel: React.FC<{
    ...
    const [sessionMenuAnchor, setSessionMenuAnchor] = useState<{ el: HTMLElement; session: WorkspaceSummary } | null>(null);
    ...
    const filteredSessions = useMemo(() => {
        if (!sessionSearchQuery.trim()) return sortedSessions;
        const q = sessionSearchQuery.toLowerCase();
        return sortedSessions.filter(s => (s.display_name || '').toLowerCase().includes(q));
    ...
    const commitRenameSession = useCallback(async () => {
        if (Date.now() - renameMountTimeRef.current < 250) {
            return;
        }
    ...
    }, [renamingSession, renameSessionDraft, sessions, activeWorkspace, cancelRenameSession, dispatch, refreshSessions, t]);
                ...
                <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', bgcolor: '#ffffff' }}>
                    ...
                    >
                        ...
                        <Tooltip title={isPinned ? t('sidebar.unpin', { defaultValue: 'Unpin' }) : t('sidebar.pin', { defaultValue: 'Pin' })} placement="bottom">
                            ...
                            >
                                <PushPinIcon sx={{ fontSize: 15, transform: isPinned ? 'none' : 'rotate(45deg)' }} />
                            </IconButton>
                        </Tooltip>
                    ...
                    <Box sx={{ px: 2, pb: 1.5 }}>
                        ...
                        >
                            ...
                            />
                            {catalogSearch && (
                                <IconButton size="small" onClick={clearCatalogSearch} sx={{ p: 0.25, color: '#94a3b8', '&:hover': { color: '#0f172a' } }}>
            ...
            {activeTab === 'sessions' && (
                <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', bgcolor: '#ffffff' }}>
                    ...
                    >
                        ...
                        <Tooltip title={isPinned ? t('sidebar.unpin', { defaultValue: 'Unpin sidebar' }) : t('sidebar.pin', { defaultValue: 'Pin sidebar' })} placement="bottom">
                            <IconButton
                                size="small"
                                onClick={onTogglePinned}
                                sx={{
                    ...
                    <Collapse in={recentExpanded}>
                        <ScrollFadeContainer sx={{ overflowX: 'hidden', overscrollBehavior: 'contain', maxHeight: 'calc(100vh - 200px)' }} resetKey={sessions.length}>
                            {filteredSessions.length === 0 ? (
                                <Box sx={{ px: 2, py: 3, textAlign: 'center' }}>
                                    <Typography sx={{ fontSize: textVar.sm, color: 'text.disabled', fontStyle: 'italic' }}>
                                        {sessionSearchQuery ? 'No matching sessions' : t('sidebar.noSessions', { defaultValue: 'No saved sessions' })}
                                    </Typography>
                                </Box>
                            ) : (
                                    ...
                                    return (
                                        <Tooltip
                                            ...
                                            title={isRenaming ? '' : (() => {
                                                ...
                                                if (isSelected) return date ? t('sidebar.currentSessionWithDate', { date }) : t('sidebar.currentSession');
                                            ...
                                            >
                                                ...
                                                {isRenaming ? (
                                                    <Box
                                                        ...
                                                        sx={{ display: 'flex', alignItems: 'center', flex: 1, gap: 0.5, mr: 0.5 }}
                                                    >
                                                        <InputBase
                                                            autoFocus
                                                        ...
                                                        <IconButton
                                                            ...
                                                            sx={{ p: 0.25, color: '#16a34a', '&:hover': { bgcolor: 'rgba(22, 163, 74, 0.1)' } }}
                                                        >
                                                        ...
                                                        <IconButton
                                                            ...
                                                            sx={{ p: 0.25, color: '#64748b', '&:hover': { bgcolor: 'rgba(0,0,0,0.05)' } }}
                                                        >
                                                ...
                                                {/* Right side: Time Ago & More Options */}
                                                {!isRenaming && (
                                                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5, flexShrink: 0 }}>
                                                        ...
                                                        >
                                                            {formatTimeAgo(s.saved_at || s.created_at)}
                                                        </Typography>
                                                        <IconButton
                                                            size="small"
                                                            onClick={(e) => {
                                                                e.stopPropagation();
                                                                setSessionMenuAnchor({ el: e.currentTarget, session: s });
                                                            }}
                                                            sx={{
                                                                p: 0.25,
                                                                color: '#94a3b8',
                                                                '&:hover': { color: '#1e293b', bgcolor: 'rgba(0,0,0,0.05)' },
                                                            }}
                                                        >
                    ...
                    >
                        <MenuItem
                            onClick={() => {
                                const targetSession = sessionMenuAnchor?.session;
                                setSessionMenuAnchor(null);
                                if (targetSession) {
                                    setTimeout(() => {
                                        startRenameSession(targetSession.id, targetSession.display_name);
                                    }, 60);
                                }
                            }}
                            sx={{ fontSize: '13px', py: 0.75, gap: 1 }}
                        >
                        ...
                        <MenuItem
                            onClick={() => {
                                ...
                                setSessionMenuAnchor(null);
                                if (targetSession) {
                                    handleExportSession(targetSession.id, targetSession.display_name);
                                }
                            }}
                            sx={{ fontSize: '13px', py: 0.75, gap: 1 }}
                        >
                            <DownloadIcon sx={{ fontSize: 16, color: '#64748b' }} />
                            {t('sidebar.exportSession', { defaultValue: 'Export' })}
                        </MenuItem>
                        <MenuItem
                            onClick={(e) => {
                                const targetSession = sessionMenuAnchor?.session;
                                setSessionMenuAnchor(null);
                                if (targetSession) {
                                    handleDeleteSession(targetSession.id, e);
                                }
                            }}
                            sx={{ fontSize: '13px', py: 0.75, gap: 1, color: '#ef4444' }}
                        >
                            <DeleteOutlineIcon sx={{ fontSize: 16, color: '#ef4444' }} />
                            {t('sidebar.deleteSession', { defaultValue: 'Delete' })}
                        </MenuItem>
                    </Menu>
                </Box>
            )}
