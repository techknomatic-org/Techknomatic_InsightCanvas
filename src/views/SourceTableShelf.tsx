...
export const SourceTableShelf: FC<{
    ...
    const handleRefreshComplete = async (newRows: any[]) => {
        ...
        setIsRefreshing(true);
        try {
            // For virtual tables (data stored on the server), sync the new
            // rows to the workspace parquet FIRST. The data grid for virtual
            // tables fetches rows from the server via SAMPLE_TABLE, so unless
            // we push the updated data to the server the grid will keep
            // showing the old rows even after Redux is updated.
            if (selectedTableForRefresh.virtual?.tableId) {
                try {
                    await apiRequest(getUrls().SYNC_TABLE_DATA, {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({
                            table_name: selectedTableForRefresh.virtual.tableId,
                            rows: newRows,
                        }),
                    });
                } catch (syncError) {
                    console.error('Failed to sync replaced data to server:', syncError);
                    // Surface the error so the user knows the replace didn't
                    // fully succeed, then bail out — don't update Redux with
                    // rows that aren't on the server.
                    dispatch(dfActions.addMessages({
                        timestamp: Date.now(),
                        type: 'error',
                        component: t('messages.dataRefresh.component'),
                        value: t('messages.dataRefresh.errorRefreshingData', { error: String(syncError) }),
                    }));
                    return;
                }
            }
