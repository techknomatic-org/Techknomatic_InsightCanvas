# Superset SSO + InsightCanvas 对接配置指南

本文档面向 **Superset 管理员**，说明如何在 Superset 中配置 SSO 登录并与 InsightCanvas（以下简称 DF）正确对接。
...
预期：页面显示"正在同步登录状态..."，然后显示"登录成功，请关闭此窗口并返回 InsightCanvas。"（因为不是从弹窗打开，没有 `window.opener`）
