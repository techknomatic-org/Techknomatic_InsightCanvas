...
InsightCanvas's desktop bundle runs the existing Flask application on a
...
On Windows and Linux, the output is `dist/InsightCanvas/`; distribute the
complete directory as a zip archive. On macOS, distribute
`dist/InsightCanvas.app`. Code signing and macOS notarization should be added
...
sign in from InsightCanvas's connector UI; the backend runs `az login` and
